...
module.exports = anwabot = async (anwabot, mek, _welkom) => {
	try {
		...
		let botNumber = anwabot.user.jid
		let ownerNumber = [`${owner}@s.whatsapp.net`, `6285157740529@s.whatsapp.net`]
		let isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let totalchat = await anwabot.chats.all()
		let groupMetadata = isGroup ? await anwabot.groupMetadata(from) : ''
		...
		let conts = mek.key.fromMe ? anwabot.user.jid : anwabot.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        let pushname = mek.key.fromMe ? anwabot.user.name : conts.notify || conts.vname || conts.name || '-'
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
        ...
        let zero = fs.readFileSync ('./anwabot/anwathumb.jpg')
...
//━━━━━━━━━━━━━━━[ BUTTON ]━━━━━━━━━━━━━━━━━//
        ...
        async function sendButLocation(id, text1, desc1, gam1, but = [], options = {}) {
            ...
            return anwabot.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
        }
...
switch (command) {
	case 'menu':
	gambar = fs.readFileSync('./anwabot/anwa.jpg')
	timestamp = speed();
...
let media = await anwabot.downloadAndSaveMediaMessage(encmedia, './database/media_user')
ran = getRandom('.webp')
await ffmpeg(`./${media}`)
.input(media)
...
anwabot.sendMessage(from, bufferlkj, audio, { mimetype: 'audio/mp4', quoted: ftrol })
fs.unlinkSync(ran)
})
break
case 'tovideo':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
anwabot.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Jangan Lupa Subscribe Zero YT7' })
fs.unlinkSync(ran)
})
break
...
let vname = anwabot.contacts[i] != undefined ? anwabot.contacts[i].vname || anwabot.contacts[i].notify : undefined
inilist.push({
"displayName": 'anwabot',
"vcard": 'BEGIN:VCARD\n'
+ 'VERSION:3.0\n'
...
hehe = await anwabot.sendMessage(from, {
...
await anwabot.sendMessage(from, buttons, MessageType.buttonsMessage, {quoted: ftrol})
break
case 'bc':
...
anwabot.sendMessage(`6285157740529@s.whatsapp.net`, options, text, { quoted: ftrol })
reply('Masalah Telah Di Laporkan Ke Owner BOT, Mohon Tunggu Untuk Proses Perbaikan')
break
case 'youtube':
teks =
`Nih Youtube Owner Ku Jangan Lupa Di Subscribe Ya https://youtube.com/anwabot`
...
anwabot.sendMessage(from, sound, MessageType.audio, {quoted: fakevo, mimetype: 'audio/mp4', ptt:true})
break
case 'sound2':
...
anwabot.sendMessage(from, aprii, video, {mimetype: 'video/mp4', quoted: ftrol})

break
case 'dosa1':				 

qute = fs.readFileSync('./anwabot/anwa.jpg') 
anwabot.sendMessage(from, qute, image, { quoted: ftrol, caption: '*SEMOGA DI BERI HIDAYAH*\nLink Download \n\nhttps://www.mediafire.com/file/h2nygxbyb6n9cyo/VID-20210107-WA1468.mp4/file' })
...
sendButLocation(from, `_*Truth*_\n\n${ttrth}`, `${tampilUcapan}`, fs.readFileSync("./anwabot/trorda.jpeg"), [ { buttonId: `${prefix}truth`, buttonText: { displayText: "➡️ NEXT" }, type: 1 } ], { contextInfo: { mentionedJid: [sender] }})                 
break
case 'dare':
...
var bff = Buffer.alloc(0)
sendButLocation(from, `_*Dare*_\n\n${der}`, `${tampilUcapan}`, fs.readFileSync("./anwabot/trorda.jpeg"), [ { buttonId: `${prefix}truth`, buttonText: { displayText: "Truth" }, type: 1 }, { buttonId: `${prefix}dare`, buttonText: { displayText: "Dare" }, type: 1} ], { contextInfo: { mentionedJid: [sender] }})
break
case 'tebakkalimat':
...
setTimeout( () => {
anwabot.sendMessage(from, ngebuff, image, { caption: '_Tebak bro!!! gak bisa jawab wajib subrek zero yt7 :v_', quoted: ftrol }) 
}, 0) // 1000 = 1s,
break
case 'suit':
...
await anwabot.sendMessage(from, ini_buffer, video, { quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Zero YT7'})
break
case 'tiktokmusic':
...
await anwabot.sendMessage(from, ini_buffer, image, { quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Zero YT7'})
break
case 'instagram':
case 'ig':
case 'igdl': 
...
anwabot.sendMessage(from, {url: res.result}, video, {quoted: ftrol})
})
break                                 
...
anwabot.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Zero YT7', thumbnail: Buffer.alloc(0)})
break
case 'asupancecan':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=${zerkey}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
anwabot.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Zero YT7', thumbnail: Buffer.alloc(0)})
break
case 'asupanhijaber':
...
reply(mess.wait)
anwabot.sendMessage(from, gambar, image, {quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Zero YT7', thumbnail: Buffer.alloc(0)})
})
break
case 'darkjoke':
...
reply(mess.wait)
anwabot.sendMessage(from, gambar, image, { quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Zero YT7'})
})
break
...
anwabot.sendMessage(from,tahta,image,{quoted:ftrol, thumbnail: Buffer.alloc(0)})
break
case 'thunder':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply('*Teks nya mana?*')
...
anwabot.sendMessage(from,thunder,image,{quoted:ftrol, thumbnail: Buffer.alloc(0)})
break
case 'blackpink':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply('*Teks nya mana?*')
...
anwabot.sendMessage(from,bp,image,{quoted:ftrol, thumbnail: Buffer.alloc(0)})
break
case 'glitch':
if (!isGroup) return reply(mess.only.group)
...
anwabot.sendMessage(from,glitch,image,{quoted:ftrol, thumbnail: Buffer.alloc(0)})
break
case 'marvel':
if (!isGroup) return reply(mess.only.group)
...
anwabot.sendMessage(from,marvel,image,{quoted:ftrol, thumbnail: Buffer.alloc(0)})
break
default:
if (isOwner) {
			if (budy.startsWith('>')) {
