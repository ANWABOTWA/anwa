//KALAU MAU SC NYA BAGUS, BELI KONTOLL
//UDAH DI KASIH GRATISAN MALAH NGELUNJAK SI ANJ, GA SADAR DIRI JEMBOTT
...
creator = 'ANWA' // GAK USAH DI UBAH ANJING HARGA CREATOR
...
var ase = new Date();
        ...
        const sticOk = (hehe) => {
			ano = fs.readFileSync('./anwabot/ok.webp')
			anwabot.sendMessage(hehe, ano, sticker, { quoted: ftrol})
		}
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `Yok Sewa LonT`, 
                            orderTitle: `Bebas Crot Dalem`,
...
reply(`「 *VIRTEX DETECTOR* 」\n\nBuat Lu Bocil Piltek, Awas Ae Lu Gua Kick Dari Group :(`)
...
JANGAN LUPA SEWA LONTE
`
gam = fs.readFileSync('./anwabot/anwa.jpg')
but = [
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
anwabot.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Jangan Lupa NgelonT' })
fs.unlinkSync(ran)
})
break
...
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
bc = await anwabot.downloadMediaMessage(encmedia)
for (let _ of anu) {
anwabot.sendMessage(_.jid, bc, image, { caption: `[ Izin Broadcast ]\n\n${body.slice(4)}` })
}
...
sendButton(from, "GOPAY : 082378206508 (Wahyudi)", faketeks, but, mek)
break
case 'dana':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
but = [
...
sendButton(from, "DANA : 085162822778 (Mustakim)", faketeks, but, mek)
break
case 'ovo':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
but = [
...
sendButton(from, "OVO : 085162822778 (Mustakim)", faketeks, but, mek)
break
case "runtime":
case "test":
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
Semoga Rejekinya Tambah Lancar Amin
    ...
    ╭───────────────❏
❍┤ヅ Gopay : 082378206508
❍┤ヅ Dana : 085162822778
❍┤ヅ Ovo : 085162822778
❍┤ヅ Pulsa : 085162822778
    ╰───────────────❏`
gam = fs.readFileSync('./anwabot/anwathumb.jpg')
but = [
          { buttonId: `${prefix}menu`, buttonText: { displayText: 'BACK TO MENU' }, type: 1 },
...
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=${anwakey}&text=${ini_txt}`).then((gambar) => {
anwabot.sendMessage(from, gambar, image, { quoted: ftrol })
})
break
...
setTimeout( () => {
anwabot.sendMessage(from, ngebuff, image, { caption: '_Tebak bro!!! gak bisa jawab wajib subrek ANWA :v_', quoted: ftrol }) 
}, 0) // 1000 = 1s,
break
case 'tembak':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
